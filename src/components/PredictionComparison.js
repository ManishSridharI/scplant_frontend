...
export default function PredictionComparison({ selectedModel }) {
      ...
      fetch(`/api/jobs/api/job_annotate_and_plot_query/`, {
        ...
        .then((data) => {
          if (data.JobAnnotateAndPlot) {
            const formattedPredictions = data.JobAnnotateAndPlot.filter((job) => job.job_celery_task_status === 'SUCCESS' && job.job_predictor === selectedModel).map((job) => ({
              id: job.id,
  ...
  const handleSubmit = async () => {
    ...
    const payload = {
      job_name: jobName,
      job_script: 3,
      job_control: controlPrediction,
      job_condition1: condition1Prediction,
    ...
    if (condition2Prediction) {
      payload.job_condition2 = condition2Prediction;
    }
  ...
  return (
    <Container>
      ...
      <Typography  sx={{ mb: 2, fontStyle: 'italic' }}>
        *Select an <Link to="/organism" style={{ color: 'blue', textDecoration: 'none' }}>Organism</Link> to view your predictions
      </Typography>
      <Typography  sx={{ mb: 2, fontStyle: 'italic' }}>
        *Choose Control and Condition Predictions (1 Control and 1 Condition are mandatory)
      </Typography>
      <DataGrid
        rows={predictions}
        columns={columns}
